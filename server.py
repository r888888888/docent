...
class _TvWorker:
    ...
    read but not the loops around them — a chatty or dribbling TV can park a
    thread indefinitely, and ``asyncio.wait_for`` giving up only abandons the
    await, never the thread. One worker caps that at a single parked thread
    rather than one per call, and keeps the connection on the only thread
    that touches it. Daemon, so a parked worker can't hold up interpreter
    exit the way a ThreadPoolExecutor's non-daemon threads would.
    """
...
async def _tv_op(fn, *, attempts: int = TV_CONNECT_ATTEMPTS, timeout: float | None = None):
    ...
    during the retry delay so other operations aren't starved.
